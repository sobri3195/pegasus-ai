...
from pegasus_ai.interface.cli import run_cli
from pegasus_ai.interface.tui import run_tui
from pegasus_ai.interface.utils import (
...
from pegasus_ai.runtime.docker_runtime import PEGASUS_AI_IMAGE
from pegasus_ai.telemetry.tracer import get_global_tracer
...
def validate_environment() -> None:  # noqa: PLR0912, PLR0915
    ...
    if not os.getenv("PEGASUS_AI_LLM"):
        missing_required_vars.append("PEGASUS_AI_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "PEGASUS_AI_LLM":
                error_text.append("• ", style="white")
                error_text.append("PEGASUS_AI_LLM", style="bold cyan")
                error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export PEGASUS_AI_LLM='openai/gpt-5'\n", style="dim white")
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  PEGASUS AI CONFIGURATION ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
def check_docker_installed() -> None:
    if shutil.which("docker") is None:
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  PEGASUS AI STARTUP ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm() -> None:
    ...
    try:
        model_name = os.getenv("PEGASUS_AI_LLM", "openai/gpt-5")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pegasus AI Multi-Agent Cybersecurity Penetration Testing Tool",
...
Examples:
  ...
  pegasus-ai --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Custom instructions
  pegasus-ai --target example.com --instruction "Focus on authentication vulnerabilities"
        """,
    )
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    panel = Panel(
        panel_content,
        title="[bold green]🛡️  PEGASUS AI CYBERSECURITY AGENT",
        title_align="center",
...
def pull_docker_image() -> None:
    ...
    if image_exists(client, PEGASUS_AI_IMAGE):
        return

    console.print()
    console.print(f"[bold cyan]🐳 Pulling Docker image:[/] {PEGASUS_AI_IMAGE}")
    ...
    with console.status("[bold cyan]Downloading image layers...", spinner="dots") as status:
        try:
            ...
            for line in client.api.pull(PEGASUS_AI_IMAGE, stream=True, decode=True):
            ...
            error_text.append("\n\n", style="white")
            error_text.append(f"Could not download: {PEGASUS_AI_IMAGE}\n", style="white")
