...
if not SANDBOX_MODE:
    raise RuntimeError("Tool server should only run in sandbox mode (PEGASUS_AI_SANDBOX_MODE=true)")
...
def agent_worker(_agent_id: str, request_queue: Queue[Any], response_queue: Queue[Any]) -> None:
    ...
    from pegasus_ai.tools.argument_parser import ArgumentConversionError, convert_arguments
    from pegasus_ai.tools.registry import get_tool_by_name
